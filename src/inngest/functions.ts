...
import { getSandbox, lastAgentTextMessageContent } from "./utils";
import { z } from "zod";
...
export const codeAgentFunction = inngest.createFunction(
  ...
  async ({ event, step }) => {
    const sandboxId = await step.run("get-sandbo-id", async () => {
    ...
    const codeAgent = createAgent<AgentState>({
      ...
      tools: [
        ...
        createTool({
          ...
          description: "Create or update the files in the sandbox",
          parameters: z.object({
            files: z.array(
              z.object({
                path: z.string(),
